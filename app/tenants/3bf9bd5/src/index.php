...
try {
    $db = new PDO(
        'mysql:host=mysql.linexa-tenant-3bf9bd5.svc.cluster.local;port=3306;dbname=linexa;connect_timeout=10',
        'linexa', 'linexapass',
    ...
    if ((int)$db->query('SELECT COUNT(*) FROM users')->fetchColumn() === 0) {
        $db->exec("INSERT INTO users (name, email) VALUES
            ('Eve',   'eve@tenant-3bf9bd5.linexa.eu'),
            ('Frank', 'frank@tenant-3bf9bd5.linexa.eu')");
    }
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge { font-family: monospace; font-size: .75rem; color: #fb923c;
             background: #1c0a00; border: 1px solid #9a3412; border-radius: 6px;
